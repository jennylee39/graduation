...
<br>
	...
	<h2>회원가입</h2><br>
	<hr style="width: 70%;">
	<form class="form" id="signupForm" method="post" name="userInfo"
		...
		<table class="type" style="margin: auto; align: center;">
			<tr>
				...
				<td>
					...
					name="user_id" id="user_id" maxlength="50" style="width: 40%;">
			...
			<tr>
				<th id="title">비밀번호</th>
				<td><input class="input" type="password" name="user_pwd"
			...
			<tr>
				<th id="title">학사호실</th>
				<td><input class="input" type="text" name="room" maxlength="50">
				</td>
			</tr>
			<tr>
				<th id="title">직업</th>
				<td><select name="job" class="box" id="job_select">
						<option value="" selected>선택하세요.</option>
						<option value="대학생-재학">대학생-재학</option>
						<option value="대학생-휴학">대학생-휴학</option>
			...
			<tr>
				<th id="title">학교</th>
				<td><input class="input" type="text" name="school"
					maxlength="50" style="width:50%;"> '-학교'까지 기입</td>
			</tr>
			<tr>
				<th id="title">학과</th>
				<td><input class="input" type="text" name="dept" maxlength="50">
				</td>
			</tr>
			...
			<tr>
				<th id="title">직책</th>
				<td><input type="radio" name="role" value="일반 학생"
					checked="checked" />일반 학생 <input type="radio" name="role"
					value="학생회 임원" />학생회 임원</td>
			</tr>
		</table>
		<br> <br>
		<button type="submit" id="submit" class="add-button" name="userInfo"
	  ...
	  success : function(data) {
	   ...
	   if(data == 1) {
		...
		function checkValue() {
			...
			if (!document.userInfo.user_pwd.value || document.userInfo.user_pwd.value ==passwdCheck){
			...
			if (!document.userInfo.name.value || document.userInfo.name.value == nameCheck) {
				alert("이름을 제대로 입력하세요.");
				return false;
			}
			//이메일 확인
			...
			if (document.userInfo.email_select.value == "1" && !document.userInfo.email2.value) {

				alert("이메일 주소를 입력하세요.");

				document.userInfo.email2.readOnly = false;
				document.userInfo.email2.value = '';
				document.userInfo.email2.focus();
				return false;
			}else if(document.userInfo.email_select.value == "1" && document.userInfo.email2.value) {
				document.userInfo.email2.readOnly = false;
				/* document.userInfo.email2.value = '';
				document.userInfo.email2.focus();
 */			}else  if(document.userInfo.email_select.value != "1" && document.userInfo.email_select.value){
				//alert("이메일 주소를 선택하세요.");
				document.userInfo.email2.readOnly = true;
				document.userInfo.email2.value = document.userInfo.email_select.value;
			//return false;	
			}
		

			if (!document.userInfo.room.value) {
			...
			if (document.userInfo.job.value == "") {
				alert("직업을 선택하세요.");
				return false;
			}
			if (!document.userInfo.phoneNum.value || document.userInfo.phoneNum.value== phonNumCheck) {
