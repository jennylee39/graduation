...
<body>
	...
	<form class="form" id="detailForm" name="detailForm">
		...
		<table class="type" style="width: 70%; min-height:30em; margin: auto; align: center;">
			...
			<tr style="height:10%;">
				<th id="title">제목</th>
				<td><span style="font-size:19px; font-weight:bold;">${notice.title}</span></td>
			</tr>
			<tr style="height:10%;">
				<th id="title">작성자</th>
				<td>${notice.writer } <span style="float:right; margin-right: 10px;font-size:14px; font-weight:bold; color:#153e73;">조회수 : ${notice.count}</span> </td>
				
			</tr>
			<c:choose>
				<c:when test="${empty notice.orgname1 and empty notice.orgname2 }">

				</c:when>
				<c:when test="${not empty notice.orgname1}">
					<tr style="height:10%;">
						<th id="title">파일 #1</th>
						<td><a href="downloadNo?bno=${notice.bno}"><c:out
									value="${notice.orgname1}"></c:out></a></td>
					</tr>
				</c:when>
				<c:when test="${not empty notice.orgname2}">
					<tr style="height:10%;">
						<th id="title">파일 #2</th>
						<td><a href="downloadNo2?bno=${notice.bno}"><c:out
									value="${notice.orgname2}"></c:out></a></td>

					</tr>
				</c:when>
				<c:when test="${not empty notice.orgname1 and not empty notice.orgname2}">
					<tr style="height:10%;">
						<th id="title">파일 #1</th>
						<td><a href="downloadNo?bno=${notice.bno}"><c:out
									value="${notice.orgname1}"></c:out></a></td>
			...
			<tr>
				...
				<td ><br> <c:choose>
						...
						<c:when
							test="${not empty notice.orgname1 and not empty notice.orgname2 }">
							<c:url value="/uploadFile\\${notice.orgname1}" var="url1"/>
							<img id="load" src="${url1}" width=50%/><br><br>
							<c:url value="/uploadFile\\${notice.orgname2}" var="url2"/>
					...
					</c:choose> <br> <c:out value="${notice.text}" escapeXml="false" /> <br><br></td>
			</tr>
			<c:choose>
				<c:when test="${empty notice.rink1 and empty notice.rink2 }">

				</c:when>
				<c:when test="${not empty notice.rink1}">
					<tr>
						<th id="title">링크 #1</th>
						<td><a href="${notice.rink1 }">${notice.rink1}</a></td>
					</tr>
				</c:when>
				<c:when test="${not empty notice.rink2}">
					<tr>
						<th id="title">링크 #1</th>
						<td><a href="${notice.rink2 }">${notice.rink2}</a></td>
					</tr>
				</c:when>
				<c:when test="${not empty notice.rink1 and not empty notice.rink2}">
					<tr>
						<th id="title">링크 #1</th>
						<td><a href="${notice.rink1 }">${notice.rink1 }</a></td>
						<th id="title">링크 #2</th>
		...
		<c:choose>
			<c:when test="${sessionScope.role eq '관리자' }">
				<input type="button" class="add-button" id="confirm_del" name="confirm_del"
					onclick="location.href='deleteNotice?bno=${notice.bno}';"
					value="삭제하기">
				<input type="button" class="add-button"
					onclick="location.href='notice';" value="목록으로 돌아가기">
			</c:when>
			<c:otherwise>
				<input type="button" class="add-button"
					onclick="location.href='notice';" value="목록으로 돌아가기">
