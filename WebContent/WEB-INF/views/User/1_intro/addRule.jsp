...
<body>
	...
	<div id="body_current" align="center">
		...
		<div style="width: 70%;">
			...
			<h4 style="float: left;">운영규정 추가</h4>
			<form method="post" id="rule" method="post" name="rule"
				onsubmit="return checkValue()" action="addRule">
				<table class="table table-bordered" id="rule"
					style="margin: auto; align: center;">
					<thead>
						<tr>
							<td><input type="text" name="id"
								style="width: 100%; background-color: #f5f5f5;" placeholder="연번"></td>
							<td><input type="text" name="content"
								style="width: 100%; background-color: #f5f5f5;"
								placeholder="금지행위"></td>
							<td><input type="text" name="minus"
								style="width: 100%; background-color: #f5f5f5;" placeholder="벌점"></td>
							...
							<td><input class="add-button"
								...
								onclick="location.href='addRule';" value="추가"></td>
						</tr>
					</thead>
				</table>
			</form>
			<br> <input type="button" class="add-button"
				style="float: right;" onclick="location.href='rule';" value="완료">

			<br>
			<br>
			<h4 style="float: left;">운영규정 수정</h4>
			<table class="table table-bordered" id="rule"
				style="margin: auto; align: center;">
				<thead>
					<tr style="border-top: 2px solid #ccc;">
						...
						<th>벌점</th>
						<th>비고</th>
						<th style="width: 10%;">수정 및 삭제</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${ruleList}" var="ruleList">
						<tr>
							<td style="width: 10%;">${ruleList.id}</td>
							<td style="width: 50%;">${ruleList.content}</td>
							<td style="width: 10%;">${ruleList.minus}</td>
							<td style="width: 10%;">${ruleList.refer}</td>
							<td style="width: 20%;"><input class="add-button"
								style="float: right; width: 50%;" name="update" type="submit"
								onclick="location.href='updateRule?id=${ruleList.id}';"
								value="수정"> <input class="add-button"
								style="float: right; width: 50%;" name="delete" type="submit"
								onsubmit="return removeCheck()"
								onclick="location.href='deleteRule?id=${ruleList.id}';"
								value="삭제"></td>
						</tr>
					</c:forEach>
				</tbody>
			...
			<c:choose>
				<c:when test="${sessionScope.role eq '관리자'}">
					<input type="button" class="add-button" style="float: right;"
						onclick="location.href='rule';" value="완료">
				</c:when>
			</c:choose>
		</div>
	</div>
	...
	<script type="text/javascript">
		function checkValue() {
			...
			if (!document.rule.id.value) {
				alert("연번을 입력하세요.");
				return false;
			}

			if (!document.rule.content.value) {
				alert("금지행위를 입력하세요.");
				return false;
			}
