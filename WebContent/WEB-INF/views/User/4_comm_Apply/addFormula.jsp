...
<br>
	...
	<h3>글쓰기</h3>
	<hr style="width: 60%;">
	<form enctype="Multipart/form-data" class="form" id="addFormula" method="post" name="addFormula"
		onsubmit="return checkValue()" action="addFormula" >
		...
		<input type="button"class="add-button" onclick="location.href='formula';" value="취소">
	</form>
	<br>
	<br>
		<!--   footer     -->
	...
	function checkValue() {
		...
		if (!document.addFormula.title.value) {
			alert("제목을 입력하세요.");
			return false;
		}

		if (!document.addFormula.text.value) {
			alert("내용을 입력하세요.");
			return false;
		}else {
			var str = $('#text').val();
		...
		if(document.addFormula.rink1.value==rink1Check){
			alert("링크 #1을 제대로 입력하세요.");
			return false;
		}
		if(document.addFormula.rink2.value==rink2Check){
			alert("링크 #2를 제대로 입력하세요.");
			return false;
		}
		
		document.addFormula.submit();
