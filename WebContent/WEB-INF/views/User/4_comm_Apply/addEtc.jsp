...
<br>
	...
	<!-- body -->
	<br>
	<h2>봉사활동 및 상담 신청하기</h2><br>
	<hr style="width: 70%;">
	<form class="form" id="addEtc" method="post" name="addEtc"
		onsubmit="return checkValue()" action="addEtc">
		...
		<tr>
			...
			<tr>
				<th id="title">제목</th>
				<td><input type="radio" name="title" value="봉사활동 신청"
					checked="checked" />봉사활동 신청 <br><input type="radio" name="title"
					value="상담 신청" />상담 신청</td>
			</tr>
			<tr>
				<th id="title">내용 및 사유</th>
				<td><textarea class="input" name="text"
						style="width: 100%; height: 30em; border:1px solid #eee "></textarea>
						신청시에 간략하게 사유를 적어주세요.<br>
						ex) ~한 이유로 봉사신청합니다, ~한 이유로 상담신청합니다.</td>
			</tr>
	...
	<script>
		...
		function checkValue() {
			...
			if (!document.addEtc.title.value) {
				alert("제목을 입력하세요.");
				return false;
			}

			if (!document.addEtc.text.value) {
				alert("내용을 입력하세요.");
				return false;
			}
			
			document.addEtc.submit();
