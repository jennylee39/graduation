...
<br>
	...
	<h2>학사 신청 수정하기</h2><br>
	<hr style="width: 70%;">
	<form id="updateDorm" method="post" name="updateDorm" onsubmit="return checkValue()" action="updateDorm?dormBno=${dormList.dormBno}">

		<table class="type" style="margin: auto; align: center;">
		<tr class="hidden"><th class="hidden" id= "title">게시글 번호</th><td>${dorm.dormBno}</td></tr>
		<tr>
				<th id="title">작성자</th>
				<td><input type="text" name="writer" value="${dormList.writer}" readonly></td>
				<td class="hidden">${dorm.id }</td>
			</tr>
			
			<tr>
				<th id="title">제목</th>
				<td><input class="input" type="text" name="title"
					maxlength="50" value="${dormList.title}" readonly></td>
			</tr>
			<tr>
				<th id="title">내용</th>
				<td><textarea class="input" name="text"
						style="width: 100%; height: 20em;">${dormList.text}</textarea></td>
			</tr>
			<tr>
				<th id="title">파일 선택 #1 </th>
				<td><input type="file" name="file"/>${dormList.orgname}
				<div class="imgs_wrap">
				</div></td>
			...
			<tr>
				...
				<td><input type="file" name="file2"/>${dormList.orgname2}</td>
			</tr> 
		

		</table>
		<br> <br>
		<button type="submit" class="add-button" onsubmit="return checkValue()" onclick="location.href='updateDorm?dormBno=${dormList.dormBno}';">작성하기</button>
		<input type="button"class="add-button" onclick="location.href='detailDorm?dormBno=${dormList.dormBno}';" value="취소">
	</form>
	<br>
	<br>
		<!--   footer     -->
	...
	<script>
		...
		function checkValue() {
			...
			if (!document.updateDorm.title.value) {
				alert("제목을 입력하세요.");
				return false;
			}

			if (!document.updateDorm.text.value) {
				alert("사유를 입력하세요.");
				return false;
			}
			
			document.updateDorm.submit();
