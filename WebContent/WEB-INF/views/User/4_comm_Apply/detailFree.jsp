	...
	<form class="form" id="deleteForm" name="deleteForm">
		...
		<table class="type"
			...
			<tr style="height: 12%;">
				...
				<td colspan="3"><span
					style="font-size: 19px; font-weight: bold;">${free.title}</span></td>
			</tr>
			<tr style="height: 12%;">
				<th id="title" style="width: 10%;">작성자</th>
				<td colspan="3">${free.writer }<span
					style="float: right; margin-right: 10px; font-size: 14px; font-weight: bold; color: #153e73;">조회수
						: ${free.count}</span>
				</td>

			</tr>
			<tr style="height: 12%;">
			<c:if test="${not empty free.orgname1}">
				<th id="title" style="width: 10%;">파일 #1</th>
				<td style="width: 40%;"><a href="downloadFr?bno=${free.bno}"><c:out
							value="${free.orgname1}"></c:out></a></td></c:if>
					<c:if test="${not empty free.orgname2}">		
				<th id="title" style="width: 10%;">파일 #2</th>
				<td><a href="downloadFr2?bno=${free.bno}"><c:out
							value="${free.orgname2}"></c:out></a></td></c:if>
			</tr>
			<c:if test="${not empty free.orgname3}">
			<tr style="height: 12%;">
				
					<th id="title" style="width: 10%;">파일 #3</th>
					<td style="width: 40%;"><a href="downloadFr3?bno=${free.bno}"><c:out
								value="${free.orgname3}"></c:out></a></td>
				
					<th id="title" style="width: 10%;">파일 #4</th>
					<td><a href="downloadFr4?bno=${free.bno}"><c:out
								value="${free.orgname4}"></c:out></a></td>
				
			</tr></c:if>
			<c:if test="${not empty free.orgname5}">
				<tr style="height: 12%;">
					<th id="title" style="width: 10%;">파일 #5</th>
					<td colspan="3" style="width: 40%;"><a href="downloadFr5?bno=${free.bno}"><c:out
								value="${free.orgname5}"></c:out></a></td>
				</tr>
			</c:if>
			<tr>

				<th id="title" style="width: 10%;">내용</th>
				<td colspan="3"><br> <c:if
						test="${not empty free.orgname1}">
						<c:url value="/uploadFile/free\\${free.orgname1}" var="url1" />
						<img id="load" src="${url1}" width=60% />
						<br>
						<br>
					</c:if> <c:if test="${not empty free.orgname2}">
						<c:url value="/uploadFile/free\\${free.orgname2}" var="url2" />
						<img id="load" src="${url2}" width=60% />
						<br>
						<br>
					</c:if> <c:if test="${not empty free.orgname3}">
						<c:url value="/uploadFile/free\\${free.orgname3}" var="url3" />
						<img id="load" src="${url3}" width=60% />
						<br>
						<br>
					</c:if> <c:if test="${not empty free.orgname4}">
						<c:url value="/uploadFile/free\\${free.orgname4}" var="url4" />
						<img id="load" src="${url4}" width=60% />
						<br>
						<br>
					</c:if> <c:if test="${not empty free.orgname5}">
						<c:url value="/uploadFile/free\\${free.orgname5}" var="url5" />
						<img id="load" src="${url5}" width=60% />
						<br>
						<br>
					</c:if> <c:out value="${free.text}" escapeXml="false" /> <br> <br></td>
			</tr>

		</table>
		<br> <br>
		<c:choose>
			<c:when test="${sessionScope.id eq free.id }">
				<input type="button" class="add-button" id="confirm_del"
					name="confirm_del"
					onclick="location.href='deleteFree?bno=${free.bno}';" value="삭제하기">
				<input type="button" class="add-button"
					onclick="location.href='free';" value="목록으로 돌아가기">
			</c:when>
			<c:otherwise>
				<input type="button" class="add-button"
					onclick="location.href='free';" value="목록으로 돌아가기">
