...
<body>
	...
	<form class="form" id="detailForm" name="detailForm" method="post"
		onsubmit="return event()" action="detailFormula?bno=${formula.bno}">


		<table class="type" style="width: 70%; margin: auto; align: center;">
			<tr class="hidden">
				<th id="title" class="hidden">게시글 번호</th>
				<td class="hidden">${formula.bno}</td>
			</tr>
			<tr>
				<th id="title">작성자</th>
				<td>${formula.writer }</td>
			</tr>
			...
			<c:if test="${not empty formula.orgname1}">
				<tr style="height: 10%;">
					<th id="title">파일 #1</th>
					<td><a href="downloadF?bno=${formula.bno}"><c:out
								value="${formula.orgname1}"></c:out></a></td>
				</tr>
			</c:if>
			<c:if test="${not empty formula.orgname2}">
				<tr style="height: 10%;">
					<th id="title">파일 #2</th>
					<td><a href="downloadF2?bno=${formula.bno}"><c:out
								value="${formula.orgname2}"></c:out></a></td>
			...
			<tr>
				<th id="title">내용</th>
				<td style="height: 15em;">${formula.text}</td>
			</tr>
			<c:choose>
				<c:when test="${empty formula.rink1 and empty formula.rink2 }">

				</c:when>
				<c:when test="${not empty formula.rink1}">
					<tr>
						<th id="title">링크 #1</th>
						<td><a href="${formula.rink1 }">${formula.rink1}</a></td>
					</tr>
				</c:when>
				<c:when test="${not empty formula.rink2}">
					<tr>
						<th id="title">링크 #1</th>
						<td><a href="${formula.rink2 }">${formula.rink2}</a></td>
					</tr>
				</c:when>
				<c:when test="${not empty formula.rink1 and not empty formula.rink2}">
					<tr>
						<th id="title">링크 #1</th>
						<td><a href="${formula.rink1 }">${formula.rink1 }</a></td>
						<th id="title">링크 #2</th>
		...
		<br> <br>
		<c:if test="${sessionScope.id eq formula.id }">
			<input type="button" class="add-button" id="confirm_del"
				name="confirm_del"
				onclick="location.href='deleteFormula?bno=${formula.bno}';"
				value="삭제하기">
		</c:if>
		<input type="button" class="add-button"
			onclick="location.href='formula';" value="목록으로 돌아가기">
