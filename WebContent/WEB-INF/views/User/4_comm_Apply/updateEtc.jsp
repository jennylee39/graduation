...
<br>
	...
	<!-- body -->
	<br>
	<h2>봉사활동 및 상담 신청 수정하기</h2><br>
	<hr style="width: 70%;">
	<form id="updateEtc" method="post" name="updateEtc" onsubmit="return checkValue()" action="updateEtc?etcBno=${etcList.etcBno}">

		<table class="type" style="margin: auto; align: center;">
		<tr><th id= "title">게시글 번호</th><td>${etc.etcBno}</td></tr>
		<tr>
				<th id="title">작성자</th>
				<td><input type="text" name="writer" value="${etcList.writer}" readonly></td>
			<td class="hidden">${etc.id }</td>
			</tr>
			
			<tr>
				<th id="title">제목</th>
				<td><input class="input" type="text" name="title"
					maxlength="50" value="${etcList.title}" readonly></td>
			</tr>
			<tr>
				<th id="title">내용 및 사유</th>
				<td><textarea class="input" name="text"
						style="width: 100%; height: 20em;">${etcList.text}</textarea></td>
			</tr>
		...
		<input type="button"class="add-button" onclick="location.href='detailEtc?etcBno=${etcList.etcBno}';" value="취소">
	</form>
	<br>
	<br>
		<!--   footer     -->
	...
	<script>
		...
		function checkValue() {
			...
			if (!document.updateEtc.title.value) {
				alert("제목을 입력하세요.");
				return false;
			}

			if (!document.updateEtc.text.value) {
				alert("사유를 입력하세요.");
				return false;
			}
			
			document.updateEtc.submit();
