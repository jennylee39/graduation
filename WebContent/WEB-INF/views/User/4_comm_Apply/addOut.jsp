...
<br>
	...
	<h2>외박 신청하기</h2><br>
	<hr style="width: 70%;">
	<form class="form" id="addOut" method="post" name="addOut"
		onsubmit="return checkValue()" style="width: 70%; margin :auto;">

		<table class="type" style="margin: auto; align: center;">
		<tr>
				<th id="title">작성자</th>
				<td><input type="text" name="writer" value="${sessionScope.name}" readonly></td>
				<th id="title" class="hidden"></th>
				<td class="hidden"><input type="text" name="id" value="${sessionScope.id}" readonly></td>
			</tr>
			
			<tr>
				<th id="title">제목</th>
				<td><input class="input" type="text" name="title"
					maxlength="50"></input>(ex. 외박 신청)</td>
			</tr>

			<tr>
				<th id="title">외박기간(박/일)</th>
				<td><input class="input" type="text" name="outTerm"
					maxlength="50">(ex. 2박 3일)</td>
			</tr>

			<tr>
				<th id="title">외박기간(날짜)</th>
				<td>
				<input name="outStart" type="date" id="currentDate" style="border: 1px solid #ccc;">
					부터 <input name="outFinish" type="date"style="border: 1px solid #ccc;">
					까지 (달력 선택)</td>
			</tr>
			...
			<tr>
				...
				<td><input name="phoneNum" value="${sessionScope.phoneNum}" ></td>
				
			</tr>
			<tr>
				<th id="title">긴급 연락처</th>
				<td><input class="input" type="text" name="emerPhone"
					maxlength="50" style="width:40%;">숫자만 기입 ('-'금지)</td>
			</tr>
			<tr>
				<th id="title">외박사유</th>
				<td><textarea class="input" name="why"
						style="width: 100%; height: 10em; border:1px solid #eee"></textarea></td>
			</tr>
	...
	<script>
		...
		function checkValue() {
			...
			if (!document.addOut.title.value) {
				alert("제목을 입력하세요.");
				return false;
			}

			if (!document.addOut.outTerm.value) {
				alert("외박기간을 입력하세요.");
				return false;
			}

			if (!document.addOut.outStart.value || !document.addOut.outFinish.value) {
				alert("외박기간을 선택하세요.");
				return false;
			}


			if (!document.addOut.place.value) {
			
				alert("목적지를 입력하세요.");
				return false;
			}

			if (!document.addOut.phoneNum.value || document.addOut.phoneNum.value==phonNumCheck) {
				alert("휴대폰번호는 숫자만 넣어주세요.");
				return false;
			}
			if (!document.addOut.emerPhone.value || document.addOut.emerPhone.value==emerNumCheck) {
				alert("긴급 연락처는 숫자만 넣어주세요.");
				return false;
			}
			if (!document.addOut.why.value) {
				alert("외박사유를 입력하세요.");
				return false;
			}
			
			document.addOut.submit();
